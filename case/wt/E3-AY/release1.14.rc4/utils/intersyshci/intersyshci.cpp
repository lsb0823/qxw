...
#ifdef  _INTERSYS_NO_THREAD_
    hci_h4_receive_msg(data, len);
    hal_intersys_start_recv(HAL_INTERSYS_ID_0);
#elif defined(__rx_queue_support__)
{
...
#endif
    ASSERT(available_len >= len, "intersys_rx_queue overflow input:%d available:%d length:%d", len, available_len, length);
    EnCQueue(&intersys_rx_queue, (unsigned char *)data, len);
    int_unlock(lock);
    hal_intersys_start_recv(HAL_INTERSYS_ID_0);
...
#endif
    ret = hal_intersys_open(HAL_INTERSYS_ID_0, HAL_INTERSYS_MSG_HCI, bt_rx, bt_tx);
    if (ret) {
