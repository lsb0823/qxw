...
int app_volume_save(uint8_t volume)
{

	struct nvrecord_env_t *nvrecord_env;
	
	nv_record_env_get(&nvrecord_env);
    nvrecord_env->volume = volume;
	nv_record_env_set(nvrecord_env);

    return 0;
}
int8_t app_volume_get(void)
{
	int8_t rev_volume = AUDIO_OUTPUT_VOLUME_DEFAULT;
	struct nvrecord_env_t *nvrecord_env;

    nv_record_env_get(&nvrecord_env);
	TRACE("## vol %d\n",nvrecord_env->volume);
	if (nvrecord_env->volume >=0  && nvrecord_env->volume <= TGT_VOLUME_LEVEL_15)
	{
		rev_volume =  nvrecord_env->volume;
	}
	return rev_volume;
